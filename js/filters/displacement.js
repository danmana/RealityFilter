var DisplacementFilters = {} || DisplacementFilters;
(function() {

	DisplacementFilters.ColorChannel = {
		RED: 0,
		GREEN: 1,
		BLUE: 2,
		ALPHA: 3
	...
	DisplacementFilters.Point = function(x, y) {
		this.x = x || 0;
		this.y = y || 0;
	};

	DisplacementFilters.DisplacementMap = function(source, map, target, point, scaleX, scaleY, channelX, channelY) {
		this.source = source;
		this.map = map;
		this.target = target;
		...
		this.point = point || new DisplacementFilters.Point();
		this.scaleX = scaleX || 0;
		this.scaleY = scaleY || 0;
		this.channelX = channelX || DisplacementFilters.ColorChannel.RED;
		this.channelY = channelY || DisplacementFilters.ColorChannel.RED;
		if (this.channelX != 0 && this.channelX != 1 && this.channelX != 2 && this.channelX != 3) this.channelX = DisplacementFilters.ColorChannel.RED;
		if (this.channelY != 0 && this.channelY != 1 && this.channelY != 2 && this.channelY != 3) this.channelY = DisplacementFilters.ColorChannel.RED;
	};

	var p = DisplacementFilters.DisplacementMap.prototype;
	...
	p.draw = function() {
		...
		while(i < pixelsLength) {
			...
			alphaValue = mapData.data[i*4+DisplacementFilters.ColorChannel.ALPHA];
